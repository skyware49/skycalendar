...
import { NavController, NavParams, AlertController, ModalController, App, Platform } from 'ionic-angular';
...
export class HomePage {
  ...
  weekList;
  noWeek : string;
  constructor(public app: App, public navCtrl: NavController, private http: Http, private alertCtrl: AlertController, public navParams: NavParams, private modalCtrl: ModalController, public platform: Platform) {
    this.getBirthList(http);
    this.getWeekList(http);
  }

  getBirthList(http) {
    let headers = new Headers({ "Content-Type": "application/x-www-form-urlencoded" });
    let options = new RequestOptions({ headers: headers });
    var addr = "http://1.221.205.250:8088/mobile/getBirth";

    http.post(addr, "", options)
      .map(res => res.json())
      .subscribe((res) => {
  ...
  getWeekList(http) {
    let headers = new Headers({ "Content-Type": "application/x-www-form-urlencoded" });
    let options = new RequestOptions({ headers: headers });
    var addr = "http://1.221.205.250:8088/mobile/getWeekList";
      ...
      .subscribe((res) => {
        ...
        if(res.getWeekList.length == 0){
          ...
          for(let index = 0; index < res.getWeekList.length; index++){
            res.getWeekList[index].schStartDate = moment(res.getWeekList[index].schStartDate).format('YYYY/MM/DD');
          }
  ...
  viewModal(week) {
    ...
    console.log(week.companyPerson);
    if (week.schAllDay == 'Y') {
      week.schAllDay = true;
    } else {
    ...
    let modal = this.modalCtrl.create('CalendarViewModalPage', {
      ...
      title: week.schTitle, content: week.schBody, memo: week.schMemo, completion: week.schCompletion, writerId: week.schWriterId, 
      company: week.schCompany, companyPerson: week.schCompanyPerson, allDay: week.schAllDay, userNm: week.userNm
    })
    ...
    modal.onDidDismiss(data => {
      if (data) {
        ...
        this.weekList.company = data.company;
        this.weekList.companyPerson = data.companyPerson;
  ...
  btnLogout() {
    var addr = "http://1.221.205.250:8088/mobile/LogoutAction";
    /*let body = {
      userId : userId,
      userPw : userPw
    };*/
