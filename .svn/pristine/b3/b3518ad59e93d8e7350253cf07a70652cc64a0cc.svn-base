...
export class CalendarPage {
  ...
  loadEvents(viewTitle) {
    var addr = "http://1.221.205.250:8088/mobile/scheduleList";
    ...
    this.http.post(addr, now, options)
      ...
      .subscribe((res) => {
        ...
        for (var index = 0; index < res.calendarList.length; index++) {
          ...
          eventData.company = res.calendarList[index].schCompany;
          eventData.companyPerson = res.calendarList[index].schCompanyPerson;
  ...
  addEvent() {
    ...
    modal.onDidDismiss(data => {
      if (data) {
        ...
        eventData.company = data.company;
        eventData.companyPerson = data.companyPerson;
  ...
  onEventSelected(event) {
    ...
    let modal = this.modalCtrl.create('CalendarViewModalPage', {
      ...
      company: event.company, companyPerson: event.companyPerson, allDay: event.allDay, userNm: event.userNm
    })
