...
export class UserJoinPage
{
  userInfo: {
    ...
    userGender: "",
    userSL: "S",
    userBirth: moment(new Date()).format('YYYY-MM-DD'),
    userAddrFullRoad: "",
    userAddrFull: "",
    userAddrDetail: "",
    userAddrSido: "",
  ...
  btnJoin()
  {
    ...
    if(this.userInfo.userSL == "양력/음력" || this.userInfo.userSL == ""){
      alert("양력/음력이 입력되지 않았습니다.");
      return false;
    }
    ...
    var addr = "http://1.221.205.250:8088/mobile/regUserAction";
  ...
  btnUserIdChk()
  {
    ...
    var addr = "http://1.221.205.250:8088/mobile/overlapIdCheck";
    let body = "userId=" + this.userInfo.userId;
    //let headers = new Headers({"Content-Type": "application/json"});
    let headers = new Headers({"Content-Type": "application/x-www-form-urlencoded"});
    let options = new RequestOptions({ headers: headers });
