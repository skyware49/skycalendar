...
export class CalendarModifyModalPage{
  event = {calendarNo: String(), startTime: String(), endTime: String(), cate1 : String(), cate2 : String(), title : String(), content : String(), memo : String(), completion : Number(), writerId : String(), company : String(), companyPerson : String(), allDay : Boolean}
  cate1 = [];
  cate2 = [];
  ...
  modify(event){
     if(event.cate1 == "" || event.cate1 == null){
      ...
      var addr = "http://1.221.205.250:8088/mobile/scheduleModify";
      let headers = new Headers({"Content-Type": "application/x-www-form-urlencoded"});
      let options = new RequestOptions({headers: headers});
    
      if(event.memo == undefined){   
        event.memo = "";
      } else if (event.company == undefined){
        event.company ="";
      } else if (event.companyPerson == undefined){
        event.companyPerson ="";
      }
